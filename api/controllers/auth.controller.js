...
export const signin = async (req, res, next) => {
    ...
    try 
    {
        ...
        const validPasswd = bcryptjs.compareSync(password, validUser.password);
        if (!validPasswd) return next(errorHandler(401, 'Wrong credentials'));
        ...
        .status(200)
        .json(rest);   // save token as session cookie
    } 
    catch (error) 
    {
        next (error);
